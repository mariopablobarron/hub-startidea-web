...
export async function uploadRoomImage(slug: string, formData: FormData): Promise<string> {
  await requireAdmin();
  ...
  // Actualiza rooms[slug].image en content.json para apuntar al nuevo path
  const next: ContentJson = {
    ...contentJson,
